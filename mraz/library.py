...
>>> def make_label(section_name, stage_name, part_name, n):
...     _, section_number = section_name.split("_")
...     _, stage_number = stage_name.split("_")
...     part_letter = part_name[0].upper()
...     label = f"{section_number}.{stage_number}.{part_letter}.{n}"
...     return label


>>> def new_show_collections(stage, stage_name):
...
...                         label = make_label(section_name, stage_name, part_name, n)
...                         if isinstance(collection, list):
...                             print(f"  {label}:")
...                             for k, segment in enumerate(collection, start=1):
...                                 print(f"    {k}: {str(segment)}")
...                         else:
...
def moment_2():
    ...
    >>> new_show_collections(section, section_name)
